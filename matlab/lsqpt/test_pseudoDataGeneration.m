%% List of Prepared Probability Distribution

filename_listEmpiDist = './ImportFiles/listEmpiDist_2valued.csv';
list_probDist = csvread(filename_listEmpiDist);
list_Nrep_doEstimation = [100, 1000, 10000, 100000];
...
size_Nrep_doEstimation = numel(list_Nrep_doEstimation);
num_rep_max = list_Nrep_doEstimation(size_Nrep_doEstimation);
...
for i_Nrep_doEstimation = 1:size_Nrep_doEstimation
    n_end = list_Nrep_doEstimation(i_Nrep_doEstimation); 
    for id = 1:num_id
        sum = zeros(num_value);
        for n = n_start:n_end
            value = data(n, id);
...
for i_Nrep_doEstimation = 1:size_Nrep_doEstimation
    Nrep = list_Nrep_doEstimation(i_Nrep_doEstimation); 
    for id = 1:num_id       
        for value = 1:num_value 
...
list_Nrep = list_Nrep_doEstimation;
set_list_empiDist_2 = set_list_empiDist_from_list_probDist_list_Nrep(list_probDist, list_Nrep, seed_x, gene_x);

for i_Nrep = 1:size_Nrep_doEstimation
    error = 0.0;
    for id = 1:num_id
        for value = 1:num_value
            diff = set_list_empiDist_2(i_Nrep, id, value) - list_probDist(id, value);
            error = error + diff * diff;    
        end
    end
    squaredError(i_Nrep) = error;
end
