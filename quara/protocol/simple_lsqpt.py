...
def check_natural_number(target: int, parameter_name: str) -> None:
    """check if ``target`` is natural number that is integer greater than 0.
...
def execute_from_csv(settings: dict) -> Tuple[np.ndarray, float]:
    ...
    check_natural_number(settings["k"], "k")

    logger.debug("--- load matL0 ---")
    matL0 = load_matL0(settings["path_matL0"], settings["dim"])
    logger.debug(matL0)
