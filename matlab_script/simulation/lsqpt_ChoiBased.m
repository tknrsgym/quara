function [Choi, obj_value, list_option] = lsqpt_ChoiBased(dim, list_state, list_povm, list_schedule, list_weight, list_empiDist, eps_sedumi, int_verbose, k, matL0, eps_logmat)
%LSQPT_CHOIBASED
...
%  
%  - k: The length of an amplified gate sequence. positive integer.
%
%  - matL0: The Hilbert-Schmidt representation of a Lindbladian in the
%           computational basis. dim^2 x dim^2 complex matrix.
%
%  - eps_logmat >= 0: a nonnegative parameter for numerical error
%                     tolerance.
%
%  Reference
...
%
    ...
    Choi_Gk     = value(varChoi);
    list_option = option.sedumi;
    
    % = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
    % Matrix k-th root
    % = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =  
    basis_comp = matrixBasis_d_comp(dim);
    HS_Gk = HS_from_Choi(Choi_Gk, basis_comp);
    HS_Lk = nonpv_log_matrix(HS_Gk, matL0, k, eps_logmat);
    HS_L = HS_Lk ./ k;
    HS_G = expm(HS_L);

    Choi = Choi_from_HS(HS_G, basis_comp);
end
