...
def load_povm_list_from_csv(
    path: str, dim: int = None, Np: int = None
) -> (int, np.ndarray):
    raw_data = np.loadtxt(path, delimiter=",", dtype=np.complex128)
...
def load_weight_list_from_csv(
    path: str, Nsche: int = None, M: int = None
) -> np.ndarray:
    raw_data = np.loadtxt(path, delimiter=",", dtype=np.float64)
    if Nsche and M:
...
if __name__ == "__main__":
    ...
    print("--- load emp list ---")
    emp_list_np = load_emp_list_from_csv(
        csv_path + "listEmpiDist_2valued.csv", dim, Nsche
    )
    emp_list_ml = matlab.double(emp_list_np.tolist())
    print(emp_list_ml)

    print("--- load weight list ---")
    weight_list_np = load_weight_list_from_csv(
        csv_path + "weight_2valued_uniform.csv", Nsche, M
    )
